...
class StagedReposFile(unittest.TestCase):
    ...
    _git_env = {
        ...
        'GIT_AUTHOR_DATE': '2005-01-01T00:00:00',
        'GIT_AUTHOR_EMAIL': 'vcs2l@example.com',
        'GIT_COMMITTER_NAME': 'vcs2l',
        'GIT_COMMITTER_DATE': '2005-01-01T00:00:00',
    ...
    def setUpClass(cls):
        ...
        for command in (
            ...
            subprocess.check_call([
                cls._git,
                *command,
            ], cwd=gitrepo_path, env=cls._git_env)
...
class StagedReposFile2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        shutil.copy(
            ...
            svnclone_path)
        for command in (
            ('checkout', 'file://' + svnrepo_path, '.', '--quiet'),
            ('add', 'LICENSE', '--quiet'),
        ...
        for command in (
            ...
            subprocess.check_call([
                cls._hg,
                *command,
            ], cwd=hgrepo_path, env=cls._hg_env)

        # Populate the staged.repos file
        repos = {
            'hg/branch': {
                'type': 'hg',
                'url': 'file://' + hgrepo_path,
                'version': 'stable',
            },
            'hg/hash': {
                'type': 'hg',
