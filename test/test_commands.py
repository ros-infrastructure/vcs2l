...
from . import StagedReposFile
from . import StagedReposFile2
...
class TestCommands(StagedReposFile):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.repos_file_url = file_uri_scheme + cls.repos_file_path
    ...
    def test_reimport(self):
        ...
        output = run_command('import', ['--skip-existing', '--input', self.repos_file_path, '.'])
        ...
        output = run_command('import', ['--force', '--input', self.repos_file_path, '.'])
        ...
        try:
            ...
            subprocess.check_output(
                [
                    ...
                    'add',
                    'origin',
                    'file://' + os.path.join(self.temp_dir.name, 'gitrepo'),
                ],
                stderr=subprocess.STDOUT,
                cwd=cwd_tag,
            )
    ...
    def test_validate(self):
        ...
        output = run_command('validate', ['--hide-empty', '--input', self.repos_file_path])
